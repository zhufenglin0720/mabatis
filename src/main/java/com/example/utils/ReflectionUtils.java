...
public class ReflectionUtils {
    ...
    public static String getPackage(@NonNull Class<?> clazz) {
        ...
        ValidUtils.assertNotNull(pck, ErrorCodeEnum.REFLECT_TYPE_PACKAGE_NOT_EXIST, getClassName(clazz));
        return pck.getName();
    }
     ...
     */
    public static String getSuperClassName(@NonNull Class<?> clazz) {
        Class<?> superClass = clazz.getSuperclass();
        ValidUtils.assertNotNull(superClass, ErrorCodeEnum.REFLECT_TYPE_PARENT_NOT_EXIST, getClassName(clazz));
     ...
     */
    public static List<String> getInterfaces(@NonNull Class<?> clazz) {
    ...
    public static List<StringBuilder> getFields(Class<?> clazz) {
        ...
        for (Field field : fields){
            list.add(append(field));
        }
        return list;
    }

    private static StringBuilder append(Field field){
        return append(Modifier.toString(field.getModifiers()),field.getType().getSimpleName(),field.getName());
    }

    private static StringBuilder append(String... filed){
        StringBuilder sb = new StringBuilder();
        for (int i = 0 ; i < filed.length ; i++){
            if(i == filed.length - 1){
                sb.append(filed[i]).append(";");
            }else{
                sb.append(filed[i]).append(" ");
            }
        }
